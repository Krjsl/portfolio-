...
// ── 12. BLOG POSTS (Static — edit this list directly to add/remove posts) ─
//
// HOW TO ADD A NEW POST:
//   Copy one of the objects below, paste it into the array, and edit the
//   title / tag / date / content fields. Save the file and upload it —
//   the new post will then appear for EVERY visitor, on every device.
//
// HOW TO DELETE A POST:
//   Just delete its entire { ... } object from the array below (including
//   the comma after it). Save and re-upload the file — the post will be
//   gone for everyone, everywhere. There is no "Delete" button on the site
//   itself; this file is the single source of truth for all visitors.
//
// FORMAT NOTES:
...
const BLOG_POSTS = [
  ...
  // ↑ Add more posts here, each separated by a comma.
];


// ── 13. BLOG RENDER (read-only, no edit/delete UI) ────────────
(function blogSystem() {
  ...
  // Give each post a stable id based on its position so the view modal
  // can find the right one when a card is clicked.
  const posts = BLOG_POSTS.map((p, i) => Object.assign({ id: 'post-' + i }, p));
  ...
  function render(filter = '') {
    ...
    const filtered = q
      ? sorted.filter(p => p.title.toLowerCase().includes(q) || (p.tag || '').toLowerCase().includes(q))
      : sorted;
    ...
    if (filtered.length === 0) {
      ...
      emptyMsg.textContent = sorted.length === 0
        ? 'No posts yet.'
